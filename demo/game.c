...
const SDL_Rect SHIELD_BOX_1 = {.x = 700, .y = 20, .w = 100, .h = 50};
const SDL_Rect SHIELD_BOX_2 = {.x = 700, .y = 50, .w = 100, .h = 50};
const size_t ROW_SEPARATION = 50;
const size_t MIN_TILES_PER_ROW = 4;
const size_t MAX_TILES_PER_ROW = 6;
...
const size_t INITIAL_LOWEST_ROW = 100;
...
const size_t SPAWN_TIME = 100000;
...
bool out_of_frame(asset_t *asset, double cam_height) {
  ...
  if (body != NULL) {
    vector_t centroid = body_get_centroid(body);
    return (centroid.y + 50 + TILE_HEIGHT / 2 < cam_height);
  }
  return false;
}
...
asset_t *activated_shield(const char *msg, const SDL_Rect box) {
  ...
  asset_t *shield_label = asset_make_text(FONT_PATH, SHIELD_BOX_1, activated_shield, TEXT_COLOR);
  return shield_label;
}
...
bool emscripten_main(state_t *state) {
  ...
  if (strcmp(state->game_state, HOME_STATE) == 0) {
      ...
      if (body != NULL) {
        if (out_of_frame(asset, state->max_cam_height - MAX.y / 2)) {
          if (asset_is_tile(asset)) {
            ...
            size_t generation_offset = 250; 

            if (state->highest_row < state->max_cam_height + generation_offset && scroll_up) { 
              spawn_row(state->max_cam_height + generation_offset + ROW_SEPARATION, state);
              state->highest_row = state->max_cam_height + generation_offset + ROW_SEPARATION;
              add_force_creators(state);
            }
    ...
    // just generating new rows and garbage collection help me
    for (size_t i = 0; i < list_size(state->body_assets); i++) {
      asset_t *asset = list_get(state->body_assets, i);
      body_t *body = get_image_asset_body(asset);
      if (body != NULL) {
        if (out_of_frame(asset, state->max_cam_height - MAX.y / 2)) {
          if (asset_is_tile(asset) ||  check_asset_info_field(asset, "player 2") || check_asset_info_field(asset, "bullet")) {
            ...
            if (state->highest_row < state->max_cam_height + generation_offset) { 
              spawn_row(state->max_cam_height + generation_offset + ROW_SEPARATION, state);
              state->highest_row = state->max_cam_height + generation_offset + ROW_SEPARATION;
              add_force_creators(state);
            }
